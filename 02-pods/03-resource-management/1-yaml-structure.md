...
|---|---|---|---|
| `resources.requests.cpu` | Important | Yes | Used by the scheduler for node placement. Expressed in millicores (e.g. `100m`). No decimal values allowed. |
| `resources.requests.memory` | Important | Yes | Minimum guaranteed memory for the container. Use binary units (`Mi`, `Gi`). |
| `resources.limits.cpu` | Optional | Yes | Maximum CPU enforced by cgroups. Throttling occurs when this limit is exceeded. No decimal values allowed. |
...
- **CPU units**: Use millicores (e.g. `100m`). Do not use decimal values.
