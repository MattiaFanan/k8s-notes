...
Kubernetes validates taint keys and values:
- Keys must begin with a letter or number, and may contain letters, numbers, hyphens, dots, and underscores, up to 253 characters. Optionally, the key can begin with a DNS subdomain prefix and a single `/`.
- Values must be 63 characters or less (if specified), alphanumeric with hyphens, underscores, and dots.
