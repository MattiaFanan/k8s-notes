...
| `csi-hostpath.vmware.com` | vSphere | vSphere volumes |
| `local-path.provisioner` | Any | Local storage on node |
...
**Note:** Only one default StorageClass can exist per cluster. If multiple are marked as default, Kubernetes will reject PVC creation.
